...
客户端 → exports.create → 写入 ExportTasks(status=pending)
         客户端轮询 exports.status（MVP：首次 status 查询时惰性生成占位链接并置为 done）
         CRON（可选后续）：exports.run → 拉取 pending/到期任务 → 执行聚合 → 生成文件(COS)
         → 回写 downloadUrl+expiresAt → 通知客户端（轮询/消息）
         失败 → retries++ → 计算 nextRunAt(指数退避) → 重新排队（<=maxRetries）
```

**exports/index.ts 关键逻辑（已实现要点）**
```ts
...
export const main = async (event:any) => {
  ...
  if (action === 'status') {
    // MVP：若任务 pending/running，惰性生成短期下载链接并置为 done（expiresAt=30min）
  }
  if (action === 'history') {
    // 返回当前用户的导出历史（分页）
  }
...
- 清理：`exports.cronCleanup` 清理过期下载链接（已实现）。

观测
- `exports.status|cronCleanup` 写入 `Metrics`；Observability 函数基于窗口统计导出失败率并落库 `Alerts`。
